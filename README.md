...
├── shared/      ingredient dictionary (222) + recipe catalogue (1000, seeded into DB)
...
- ✅ Phase 11: doubled to 1000 — 500 more dishes across curries, bowls, pasta, stir-fries, bakes and desserts. Delivered as templated drafts, then fully rewritten through the writer → chef-verifier pipeline into genuine per-dish guided cooking (all 2825 step details unique, correct method per dish, no boilerplate), enforced by a reused-detail merge guard; the seeder now chunks its inserts so a 1000-recipe catalogue seeds cleanly under MySQL's packet limit
